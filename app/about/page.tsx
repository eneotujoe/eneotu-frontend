          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            {[
              ...
              <div key={item.school} className="soft-shadow-inset rounded-2xl p-6 text-center">
                <div className="font-bold text-primary pb-5">{item.school}</div>
                <div className="text-sm text-muted-foreground">{item.certificate}</div>
              </div>
            ))}
          </div>

          <div className="text-2xl md:text-3xl font-bold text-primary mt-10 pb-5">Award</div>
          <div className="grid grid-cols-1 gap-4 mb-5">
            {[
              { school: "Constanta Maritime University, Romania", certificate: "Best student award" },
            ].map((item) => (
              <div key={item.school} className="soft-shadow-inset rounded-2xl p-6 text-center">
                <div className="font-bold text-primary pb-5">{item.certificate}</div>
